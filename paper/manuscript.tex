...
from the Diebold-Mariano test with the Harvey correction at each horizon. A decision
criterion was fixed before the analysis. We further varied training history while holding
test dates fixed, and evaluated monthly minimum temperature as an exogenous covariate
under a leakage-free design.
...
$p<0.05$. Holding test dates fixed, nine additional years of training reduced sMAPE by
...
unaffected ($p=0.42$), and TimesFM then led.
...
S\~ao Paulo has one of the highest death registration completeness rates in Brazil
\missing{reference and figure for death registration completeness in S\~ao Paulo; the
value stated in an earlier draft was above 95\%, but we could not locate the source and
it is not asserted here without one}. We used raw SIM data without garbage code
redistribution, which should be considered when comparing against Global Burden of
Disease estimates.
...
makes the paired analysis below valid. Accuracy was summarised by mean absolute error
(MAE), root mean squared error (RMSE) and symmetric mean absolute percentage error (sMAPE).
...
One caveat applies to a single row of Table~\ref{tab:desempenho}. XGBoost is deterministic
within a fixed environment, but its accuracy on this task shifts with the library version
and with the number of threads used for fitting: across six released versions and five
thread counts, holding the code, the data and the seed constant, its sMAPE ranged from
6.83\% to 6.98\%. CatBoost was stable to the tenth of a basis point under the same
variation. The XGBoost figure reported here should therefore be read as accurate to roughly
a tenth of a percentage point rather than to the digits shown. No conclusion depends on
that precision: in every environment tested, XGBoost remained the weakest of the five
models, remained worse than the seasonal naive benchmark, and remained more than two
percentage points behind the leading three.

\section{Results}

\subsection{Aggregate performance}

\input{tables/tab1_desempenho}
...
1.275, or 6.59\% and 6.94\% in sMAPE.
...
The same procedure applied to XGBoost moved it 0.23 pp in the wrong direction, leaving it
worse than its own untuned configuration.
\aberto{The tuning table generated by the revision, \texttt{tables/revisao\_tab\_optuna},
reports 0.21 pp for this movement. The two numbers measure the same thing against different
baselines: 0.23 against the 6.83\% of the original run, 0.21 against the 6.85\% of the
environment in which the search ran. Decide which baseline the text should use and make the
two agree. That table is not currently \texttt{\textbackslash input} anywhere in this
document, so the tuning study reaches the reader only as prose; decide whether it should
be included.}
...
yields 6.35\%. Even that leaked ceiling remains behind the seasonal naive method and some
1.5 pp behind the three leading approaches.
\aberto{This holds for CatBoost, whose ceiling of 6.35\% is worse than the 6.27\% of the
seasonal naive method, but not for XGBoost, whose ceiling of 6.22\% in the
tuning table is ahead of it by 0.045 pp. As written the sentence reads as a claim
about both. Either restrict it to CatBoost or state the XGBoost exception, which does not
change the argument: a bound attainable only with the test set in hand is not a result.}
...
0.32 to 0.79 percentage points for CatBoost and from 0.67 to 1.28 for XGBoost, and
...
It reaches an sMAPE of 6.04\%, ahead of CatBoost by 0.55 percentage points and of XGBoost by
0.90, making it the strongest tabular model tested here. It sits ahead of both naive
references on the point estimate, at 6.18\% and 6.27\%, and behind the three leading models
by more than 1.2 percentage points.

That last comparison is the one that carries. The advantage over the naive references is
0.14 percentage points, and this paper does not treat a point estimate as a result: the
paired test that Table~\ref{tab:variantes} applies to every feature engineering variant
could not be run here, because the per-forecast records for this run were not available. The
caution is not procedural. The best boosting variant reached a \emph{larger} margin over the
seasonal naive method, 0.18 percentage points, and still failed that test with an interval
of $[-0.45, +0.10]$ and no significant Diebold-Mariano cell. The honest summary is that
TabPFN is the best tabular model in this comparison and that whether it beats a rule
requiring no model remains untested.
...
distance from TabPFN to Prophet is 1.33 percentage points, wider than the entire spread
among the three leading models. What the boosting results were showing was not the absence
...
foundation model does not, and overtakes them in that regime.}
\label{fig:janela}
\end{figure}
...
\subsection{Temperature helps, below the confirmation threshold}

\input{tables/tab5_temperatura}
...
explanation is wrong. Repeating the comparison with Fourier calendar terms added to the
feature set shrinks the CatBoost gain from 0.258 to 0.076 percentage points,
...
\begin{figure}[htbp]
\centering
\input{figures/fig6_temperatura}
...
Positive values favour the model with temperature. All three intervals exclude zero, which
is why the effect is reported as suggestive rather than absent, and none clears the
pre-declared horizon-level threshold, which is why it is not reported as established.}
\label{fig:temp}
\end{figure}

All three gains in Table~\ref{tab:temperatura} and Figure~\ref{fig:temp} have intervals
excluding zero, but none reaches significance in at least three horizons, so under the
pre-declared criterion the effect is suggestive and not established. The ceiling scenario
is more informative than the
point gains: replacing the climatological future covariate with the true observed future
temperature, which no operational forecast could access, improves SARIMA by a further
0.03 pp only. The predictive content of monthly temperature in this series is therefore
almost entirely its climatology, information a seasonal model already carries.

\subsection{Seasonality}

\begin{figure}[htbp]
\centering
\input{figures/fig5_sazonal}
...
\textbf{Author contributions (CRediT).} \missing{author contribution statement per CRediT
taxonomy, to be assigned by the authors}

\textbf{Funding.} \missing{funding sources and grant numbers, or a statement that the work
...
and no contact with participants. \missing{confirmation of institutional review board waiver
or approval number, if the institution requires one for public secondary data}
...
\textbf{Acknowledgements.} \missing{acknowledgements, including computational resources and
any non-author contributors}
