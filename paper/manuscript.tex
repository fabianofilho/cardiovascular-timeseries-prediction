...
Only models that natively accept exogenous regressors received the covariate (SARIMA,
XGBoost, CatBoost). Prophet and TimesFM were excluded from that comparison rather than
being given an input they would ignore.
...
\subsection{Two models fail to beat a naive benchmark}

A forecast is informative only relative to a rule that requires no model, and the scaled
error in Table~\ref{tab:desempenho} makes that comparison explicit. Prophet, SARIMA and
TimesFM reach MASE values of 0.860, 0.880 and 0.887, each reducing the error of the
seasonal naive method by roughly 11 to 14\%. CatBoost and XGBoost reach 1.213 and 1.275:
both are worse than simply repeating the same month of the previous year.

The size of that gap is easier to read in the original units. The seasonal naive method
attains an sMAPE of 6.27\%, and adding a drift term brings it to 6.18\%, while CatBoost and
XGBoost record 6.59\% and 6.94\%. The two gradient boosting models are therefore not merely
the weakest of the five; they fall below the benchmark that any model has to clear to
justify being fitted at all. The unstructured naive method, which repeats the last observed
month and ignores the annual cycle, is far behind everything else at 10.74\%, and the
distance between it and the seasonal naive method measures how much of this series is
seasonal.
...
worse than its own untuned configuration. Repeating the CatBoost search over 60 trials with
...
1.5 pp behind the three leading approaches. The limitation is not the size of the search.
...
sufficient for fine-grained operational decisions, and the absence of prediction intervals
discussed below is the binding limitation for that use.

\subsection{Limitations}

\textbf{No prediction intervals, and therefore no forecast calibration.} All five models
here produce point forecasts, and all three metrics are point-forecast metrics. What we
quantified is the uncertainty of the accuracy metric, not the uncertainty of an individual
forecast. Empirical coverage against a nominal level was therefore never assessed. For a
manager sizing capacity, the interval is the operative quantity, and its absence is the most
consequential gap in this work. Enabling the intervals each model already supports natively
and measuring coverage by horizon is the immediate next step.
