...
- Horizonte: `6`
- Janela mínima de treino: `60` (5 anos; ≥4 ciclos sazonais efetivos após a dupla diferenciação do SARIMA. Nota: a rodada 2019–2023 usou 24 no README/paper e 36 no default do script — divergência histórica registrada aqui, não corrigida em README/paper por depender da decisão de LAB-64)
...
- Mudança de protocolo relevante: o clamp `[0.1·min, 3·max]` que existia só em SARIMA e boosting foi **removido** (era assimétrico — Prophet e TimesFM nunca o tiveram). Nenhuma previsão desta rodada disparou o warning diagnóstico que o substituiu (0 ocorrências no log), então o efeito da remoção nesta série foi nulo.
...
- **Top-3 continua empate técnico**: as três diferenças pareadas entre prophet/sarima/timesfm têm IC contendo zero (p de 0,17 a 0,77) e nenhum Diebold-Mariano abaixo de 0,05 em nenhum dos 6 horizontes. O ranking pontual **inverteu** em relação à rodada anterior (Prophet passou de último a primeiro) — confirmação adicional de que a ordem de 2019–2023 era ruído.
...
Nove anos extras de treino compram 1,2–1,7 pp de sMAPE nas mesmas datas — maior que toda a amplitude entre modelos da rodada anterior (0,38 pp). O sMAPE agregado de 4,7–4,8% não é comparável diretamente com os 7,3% antigos porque 2015–2019 foi mais fácil de prever que 2021–2023.
...
2. Entre Prophet, SARIMA e TimesFM continua **não havendo vencedor** — com o dobro de precisão de medida. A escolha entre eles segue operacional (custo, robustez, interpretabilidade), não estatística.
3. XGBoost/CatBoost com defasagens e sem exógena são inferiores com significância em todos os horizontes — fecha a pergunta aberta em LAB-63 para o caso univariado. Ver `benchmark_boosting_temperatura.md` para o caso com temperatura.
